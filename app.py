...
db=mysql.connector.connect(user="root",port='3307',database='cyber_hacking')
cur=db.cursor()
...
def view():
    ...
    le=LabelEncoder()
    dataset.replace({'hacked':1,'poor security':0,'lost / stolen media':0,'inside job':0,'accidentally published':0,'lost / stolen computer':0,'unknown':0,'poor security/inside job':0,'improper setting, hacked':0,
    ...
    dataset['Entity'] = le.fit_transform(dataset['Entity'])
    dataset['Organization type'] = le.fit_transform(dataset['Organization type'])
